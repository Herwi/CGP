...
vec3 albedo = texture2D(albedoMap, interpTexCoord).rgb;
float roughness = texture2D(roughnessMap, interpTexCoord).r;
float metallic = texture2D(metallicMap, interpTexCoord).r;
